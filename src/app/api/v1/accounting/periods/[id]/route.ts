...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    return await withRole(req, ['admin'], async (payload) => {
      ...
      if (!period) return withSecurityHeaders(NextResponse.json({ success: false, error: 'Period not found' }, { status: 404 }));

      const { ipAddress, userAgent } = getClientInfo(req);
      const tenantId = getTenantId() ?? DEFAULT_TENANT_ID;

      if (action === 'close') {
        if (period.status !== 'open') return withSecurityHeaders(NextResponse.json({ success: false, error: 'Period is not open' }, { status: 400 }));
        ...
        return withSecurityHeaders(NextResponse.json({ success: true, data: { message: 'Period closed successfully' } }));
      }

      if (action === 'reopen') {
        if (period.status !== 'closed') return withSecurityHeaders(NextResponse.json({ success: false, error: 'Period is not closed' }, { status: 400 }));
        ...
        return withSecurityHeaders(NextResponse.json({ success: true, data: { period: updated } }));
      }

      if (action === 'lock') {
        if (period.status !== 'closed') return withSecurityHeaders(NextResponse.json({ success: false, error: 'Period must be closed first' }, { status: 400 }));
        const updated = await prisma.accountingPeriod.update({
          where: { id },
          data: { status: 'locked' },
        });
