  ...
  try {
    return await withRole(req, ['admin', 'staff'], async () => {
      ...
      const rows = products.map((p) => ({
        ...
        'Price (EGP)': parseFloat(String(p.price)) || 0,
        'Cost (EGP)': parseFloat(String(p.costPrice || 0)),
        ...
        'Tax Rate': p.taxRate != null ? parseFloat(String(p.taxRate)) : 14,
        'Stock Value': (parseFloat(String(p.costPrice || 0)) * Number(p.stock)),
        'Active From': p.activeFrom ? new Date(p.activeFrom).toLocaleDateString() : '',
